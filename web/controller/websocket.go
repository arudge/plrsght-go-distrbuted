...
func (wsc *websocketController) listenForSources() {
	...
	for msg := range msgs {
		...
		fmt.Printf("Sending a source data message for sensor name: %", sensor.Name)
...
func (wsc *websocketController) listenForMessages() {
	...
	fmt.Println("Looking for messages on WebappReadingsExchange")

	for msg := range msgs {
		fmt.Println("Read message on WebappReadingsExchange")
...
func (wsc *websocketController) sendMessage(msg message) {
	...
	fmt.Println("Writing message to sockets.......")
	for _, socket := range wsc.sockets {
		err := socket.WriteJSON(msg)

		if err != nil {
			fmt.Println("Error on writing removing socket")
...
func (wsc *websocketController) listenForDiscoveryRequests(socket *websocket.Conn) {
	fmt.Println("Listening for discovery requests")
	for {
		msg := message{}
		...
		fmt.Println("Got a message on the socket")

		if msg.Type == "discover" {
			fmt.Println("It was a discover message")
			wsc.ch.Publish(
				"", //exchange string,
